...
def pad(img, headpad, padding):
    ...
    if padding>0:
        ...
        ids = np.where(np.sum(logi_matrix, axis=0) == img.shape[0])
        if ids[0].tolist() != []:
...
if __name__=='__main__':
    label_root = r'E:\HWDB2.0-2.2\HWDB2.xTest_label'
    pages_root = r'E:\HWDB2.0-2.2\HWDB2.xTest_images'
    label_det = r'E:\HWDB2.0-2.2\HWDB2.xTest_fullLabels'
    pages_det = r'E:\HWDB2.0-2.2\HWDB2.xTest_fullpages'
    ...
    for ds in tqdm(pages_set):
        ...
        seg_sorted = strsort([d for d in pages_for_set if ds == d.split('_')[0]])
